...
The iOS client mirrors the deployed community-event contracts for listing, detail, attendance, and authorized creation/update/deletion. `Community/getMyEvents` backs a cross-community native agenda. Its first page works, but the strict live validator rejects the declared `beforeId` cursor; backend issue #67 tracks pagination and the app does not send the rejected field. Event `call` and `broadcast` records can be read, but joining them remains intentionally scoped to the later calls workstream. The app-store runtime uses the signed `Plugins/pluginRequest` route instead of exposing the user's API session to plugin JavaScript. Each plugin receives a non-persistent WebKit data store and a narrow message bridge; identity, iframe origin, declared permissions, navigation, media capture, and request rate are enforced natively.
