...
The iOS client mirrors the deployed community-event contracts for listing, detail, attendance, and authorized creation/update/deletion. `Community/getMyEvents` backs a cross-community native agenda and now paginates incrementally with the `(scheduledBefore, beforeId)` cursor fixed by backend PR #68 / issue #67. Pages are deduplicated defensively and pull-to-refresh resets the cursor. Backend issue #69 tracks the remaining requirement that first-page queries always include the UUID ordering tiebreaker used by cursor pages. Event `call` and `broadcast` records can be read, but joining them remains intentionally scoped to the later calls workstream. The app-store runtime uses the signed `Plugins/pluginRequest` route instead of exposing the user's API session to plugin JavaScript. Each plugin receives a non-persistent WebKit data store and a narrow message bridge; identity, iframe origin, declared permissions, navigation, media capture, and request rate are enforced natively.
