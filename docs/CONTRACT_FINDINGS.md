...
Backend PR #73 / issues #71 and #72 made community and user article lists deterministic across timestamp ties by adding `beforeId`/`afterId` cursor tiebreakers and ordering every page by `(timestamp, articleId)`. The same release added `communityTags`/`anyCommunityTags` to `Community/getArticleList`, so the Feed topic filter now applies to containing-community topics exactly as `Community/getUpcomingEvents` does. Article-level `tags`/`anyTags` remain separate. The iOS Feed uses `(publishedBefore, beforeId)` for newest-first articles, `(scheduledAfter, afterId)` for oldest-first upcoming events, batch-resolves community identities through `Community/getCommunitiesById`, and defensively deduplicates pages without relying on that for correctness.
